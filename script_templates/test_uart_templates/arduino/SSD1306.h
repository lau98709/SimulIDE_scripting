...
// Fonction pour envoyer une commande au SSD1306
...
// Fonction pour envoyer des données au SSD1306
void ssd1306_data(uint8_t data) {
  Wire.beginTransmission(SSD1306_I2C_ADDRESS);
  Wire.write(0x40); // Co = 0, D/C# = 1 (donnée)
  Wire.write(data);
  Wire.endTransmission();
}

// Définition de la police 5x7 pour tous les caractères ASCII (0 à 127)
const uint8_t font5x7[256][5] PROGMEM = {
...
// Fonction pour initialiser l'écran SSD1306
void ssd1306_init() {
  ssd1306_command(0xAE); // Écran OFF
  ssd1306_command(0xA8); // Réglage du multiplexeur
  ssd1306_command(0x3F); // Rapport 1/64
  ssd1306_command(0xD3); // Décalage d'affichage
  ssd1306_command(0x00);
  ssd1306_command(0x40); // Ligne de départ d'affichage
  ssd1306_command(0xA1); // Remappage des segments
  ssd1306_command(0xC8); // Sens de balayage des COM
  ssd1306_command(0xDA); // Configuration du matériel COM
  ssd1306_command(0x12);
  ssd1306_command(0x81); // Contraste
  ssd1306_command(0x7F);
  ssd1306_command(0xA4); // Affichage normal
  ssd1306_command(0xA6); // Mode normal
  ssd1306_command(0xD5); // Réglage de l'oscillateur
  ssd1306_command(0x80);
  ssd1306_command(0x8D); // Activation de la charge
  ssd1306_command(0x14);
  ssd1306_command(0xAF); // Écran ON
}

// Fonction pour effacer l'écran
void clear_display() {
  for (uint8_t page = 0; page < 8; page++) {
    ssd1306_command(0xB0 + page); // Sélection de la page
    ssd1306_command(0x00); // Adresse de colonne basse
    ssd1306_command(0x10); // Adresse de colonne haute
    for (uint8_t col = 0; col < 128; col++) {
      ssd1306_data(0x00); // Remplit avec des pixels éteints
    }
  }
}

// Fonction pour afficher un caractère à une position donnée (modifiée pour PROGMEM)
void display_char(uint8_t x, uint8_t y, char ch) {
//  if (ch < '0' || ch > '9') return; // Ne supporte que les chiffres 0-9
//  uint8_t index = ch - '0';
  
  uint8_t index = ch;

  ssd1306_command(0xB0 + y);          // Sélection de la page
  ssd1306_command(0x00 + (x & 0x0F));    // Adresse de colonne basse
  ssd1306_command(0x10 + ((x >> 4) & 0x0F)); // Adresse de colonne haute

  for (uint8_t i = 0; i < 5; i++) {
    // Lecture depuis PROGMEM avec pgm_read_byte
    ssd1306_data(pgm_read_byte(&font5x7[index][i])); 
  }
  ssd1306_data(0x00); // Espace entre les caractères
}

// Fonction pour afficher une chaîne de caractères à une position donnée
void display_string(uint8_t x, uint8_t y, const char *str) {
  while (*str) {
    display_char(x, y, *str);
    x += 6; // Avance de 6 pixels (5 pour le caractère + 1 espace)
    if (x + 5 >= 128) {
      x = 0;    // Retour à la première colonne
      y += 1;  // Passe à la page suivante
    }
    str++;
  }
}
...
void setup() {
  ...
  ssd1306_init(); // Initialisation de l'écran SSD1306
  clear_display(); // Effacement de l'écran
}

void loop() {
  display_string(0, 0, "1234567890"); // Affichage d'une chaîne de caractères
